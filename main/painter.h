...
namespace Mcucpp
{
	namespace Graphics
	{
		...
		public:
			...
			Color GetBackColor() { return _backColor; }
		...
		void Painter<Display>::DrawLine(Coord x1, Coord y1, Coord x2, Coord y2)
		{
			...
			Coord length = std::max(deltaX, deltaY);
		...
		{
			r += _penWidth >> 1;
			Coord rdiff = std::max(r - _penWidth, 0);
			Coord sqrRInt = rdiff * rdiff;
			...
			while (dx <= dy)
			{
				...
				else
				{
					if (old_dy != dy && dx != dy)
					{
						Coord dxLim = sqrRInt - dy * dy;
						for (Coord i = 0; (dx - i) * (dx - i) > dxLim && dx >= i; i++)
						{
							CircleHelper(x, y, dx - i, dy);
						}
					}
					Coord dyLim = sqrRInt - dx * dx;
					for (Coord i = 0; (dy - i) * (dy - i) > dyLim && dy >= i; i++)
					{
